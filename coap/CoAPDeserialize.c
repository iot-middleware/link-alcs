...
{
    if(COAP_MSG_MAX_TOKEN_LEN < msg->header.tokenlen){
        return -1;
    }
...
int CoAPDeserialize_Options(CoAPMessage *msg, unsigned char *buf, int buflen)
{
    ...
    msg->optcount = 0;
    while ((count < buflen) && (0xFF != *ptr) && (index < COAP_MSG_MAX_OPTION_NUM)) {
...
int CoAPDeserialize_Message(CoAPMessage *msg, unsigned char *buf, int buflen)
{
    ...
    if (buflen < 4 || COAP_MSG_MAX_PDU_LEN < buflen) {
    ...
    if(-1 == count){
        return COAP_ERROR_WEIRD_PACKET;
    }
    ptr += count;
    remlen -= count;
