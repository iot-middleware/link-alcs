...
#define COAP_ERROR_WEIRD_PACKET                (COAP_ERROR_BASE | 15)
...
typedef struct
{
    unsigned short       send_maxcount;  /*list maximal count*/
    unsigned short       obs_maxcount;   /*observe maximal count*/
    unsigned short       port;           /* Local port */
    char                 *group;         /* Multicast address */
    unsigned int         waittime;
    CoAPEventNotifier    notifier;
    void                 *appdata;
    unsigned short       res_maxcount;
}CoAPInitParam;
...
extern int CoAPMessageId_get(CoAPMessage *message, unsigned short *msgid);

extern int CoAPMessageType_set(CoAPMessage *message, unsigned char type);

extern int CoAPMessageType_get(CoAPMessage *message, unsigned char *type);
...
extern int CoAPMessageToken_get(CoAPMessage *message, unsigned char *token,
        unsigned char *tokenlen);
...
extern int CoAPMessagePayloadlen_get(CoAPMessage *message, unsigned short *len);

extern int CoAPMessagePayload_get(CoAPMessage *message,
...
extern int CoAPResource_unregister(CoAPContext *context, const char *path);
/*CoAP observe APIs*/
