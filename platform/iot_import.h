...
#define _IN_OPT_            /* å‚æ•°å‰ç¼€,    è¡¨ç¤ºåŽé¢è·Ÿç€çš„å‚æ•°æ˜¯ä¸€ä¸ªå¯é€‰çš„è¾“å…¥å‚æ•° */
#define _OU_OPT_            /* å‚æ•°å‰ç¼€,    è¡¨ç¤ºåŽé¢è·Ÿç€çš„å‚æ•°æ˜¯ä¸€ä¸ªå¯é€‰çš„è¾“å‡ºå‚æ•° */
...
/**
 * @brief Destroy the specified semaphore object, it will release related resource.
 *
...
/**
 * @brief   »ñÈ¡Éè±¸µÄ`ProductKey`, ÓÃÓÚ±êÊ¶Éè±¸µÄÆ·Àà, ÈýÔª×éÖ®Ò»
 *
 * @param   product_key : ÓÃÀ´´æ·ÅProductKey×Ö·û´®µÄÊý×é
 * @return  Ð´µ½product_key[]Êý×éÖÐµÄ×Ö·û³¤¶È, µ¥Î»ÊÇ×Ö½Ú(Byte)
 */
int HAL_GetProductModel(_OU_ char model[PRODUCT_KEY_MAXLEN]);


/**
 * @brief   »ñÈ¡Éè±¸µÄ`DeviceID`, ÓÃÓÚ±êÊ¶Éè±¸µ¥Æ·µÄID
 *
 * @param   device_id : ÓÃÀ´´æ·ÅDeviceID×Ö·û´®µÄÊý×é
 * @return  Ð´µ½device_id[]Êý×éÖÐµÄ×Ö·û³¤¶È, µ¥Î»ÊÇ×Ö½Ú(Byte)
 */
int HAL_GetDeviceID(_OU_ char deviceID[DEVICE_ID_MAXLEN]);
