...
int clock_gettime_mach(int clk_id, struct timespec *t){
...
{
    sem_t * sem = sem_open("HAL_SemaphoreCreate", O_CREAT, 0644, 1);
    if (sem == SEM_FAILED) {
        return NULL;
    }
    return sem;
...
int HAL_SemaphoreWait(_IN_ void *sem, _IN_ uint32_t timeout_ms)
{
    ...
    do {
        int tmpRet = 0;
#ifdef __APPLE__
        tmpRet = clock_gettime_mach(CLOCK_REALTIME, &ts);
#else
        tmpRet = clock_gettime(CLOCK_REALTIME, &ts);
#endif
        if (tmpRet == -1) {
            return -1;
        }
