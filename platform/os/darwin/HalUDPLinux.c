...
void HAL_Get_broadcast_ip (char ip[16])
{
    ...
    do { 
        ...
        if (ioctl(socket_id, SIOCGIFBRDADDR, &ifr) < 0) {
            close(socket_id);
            break;
        }
...
intptr_t HAL_UDP_create(const char *host, unsigned short port)
{
    ...
    ret = setsockopt(socket_id, SOL_SOCKET, SO_REUSEPORT, &flag, sizeof(flag));
    if(ret < 0)
    {
        close(socket_id);
    ...
    flag = 1;
    if ((ret = setsockopt (socket_id, SOL_SOCKET, SO_BROADCAST, &flag, sizeof(flag))) < 0) {
