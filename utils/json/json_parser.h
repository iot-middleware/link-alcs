...
typedef int (*alcs_json_parse_cb)(char *p_cName, int iNameLen, char *p_cValue, int iValueLen, int iValueType,
                             void *p_Result);
...
* @note None.
**/
int alcs_json_parse_name_value(char *p_cJsonStr, int iStrLen, alcs_json_parse_cb pfnCB, void *p_CBData);
...
* @note None.
**/
char *alcs_json_get_value_by_name(char *p_cJsonStr, int iStrLen, char *p_cName, int *p_iValueLen, int *p_iValueType);
...
* @note None.
**/
char *alcs_json_get_value_by_name_len(char *p_cJsonStr, int iStrLen, char *p_cName, int p_cNameLen, int *p_iValueLen,
                                 int *p_iValueType);
 ...
 * @note None.
 */
char *alcs_json_get_object(int type, char *str, char *str_end);
char *alcs_json_get_next_object(int type, char *str, char *str_end, char **key, int *key_len, char **val, int *val_len,
                           int *val_type);
...
#define json_object_for_each_kv(str, slen, pos, key, klen, val, vlen, vtype) \
    for (pos = alcs_json_get_object(JOBJECT, str, str + slen); \
         pos != NULL && *pos!= 0 && (pos=alcs_json_get_next_object(JOBJECT, pos, str + slen , &key, &klen, &val, &vlen, &vtype))!=0; )
...
#define json_array_for_each_entry(str, slen, pos, entry, len, type) \
    for (pos = alcs_json_get_object(JARRAY, str, str + slen); \
         pos != NULL && *pos!= 0 && (pos=alcs_json_get_next_object(JARRAY, ++pos, str + slen, 0, 0, &entry, &len, &type))!=0; )
